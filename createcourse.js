$(document).ready(function () {
    ...
    $("#addcourse").click(function() {
        $("body").append($('<form action="submit.php" method="post"><label for="courseid">Course ID:</label><input type="text" id="courseid" name="courseid"><input type="submit" id="addsection" value="Add Section"><div id="section"><h3>Section 1</h3>'))
        $("#section").append($('<label for="date">').text("Date:"));
        $("#section").append($('</label>'));
        $("#section").append($('<select name="date" id="date"><option value="monday">').text("Monday"));
        $("#section").append($('</option><option value="tuesday">').text("Tuesday"));
